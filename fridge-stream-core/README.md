...
Use a **transparent** Webpage source. Chat is a separate source so you can place and size it on its own. Add `?badges=0` to hide platform letters on combined chat.
...
- Logs every chat message to SQLite (`data/stream_core.db`)
- In-house **chat points** (configurable per-message + cooldown)
- Link Kick / YouTube (etc.) identities so one person keeps one balance
- Give / take points, notes, merge accounts
- Download full chat history as CSV (all or per user)
- **Config tab** — simple GUI for `config.yaml` + `commands.json`
  ...
  - Commands list with edit / add / delete
  - Saves the same files tech users edit by hand (hybrid approach)
  - After save: **restart Stream Core** for changes to apply
...
│   └── admin_routes.py     # points, chat export, config/commands editor
├── admin/                  # dashboard UI (Users, Chat, Config tabs)
